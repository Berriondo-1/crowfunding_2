...
<div class="px-4 pt-6 pb-10 lg:px-8 space-y-6">
    ...
    <div class="grid gap-4 sm:grid-cols-3">
        <div class="rounded-2xl border border-white/10 bg-zinc-900/80 p-4">
            <p class="text-[11px] uppercase tracking-[0.2em] text-zinc-400">Calificacion promedio</p>
        ...
        <div class="rounded-2xl border border-white/10 bg-zinc-900/80 p-4">
            ...
            <p class="mt-2 text-2xl font-bold text-emerald-200">USD {{ number_format($totalProveedor, 2) }}</p>
            <p class="text-xs text-zinc-500">Pagos liquidados a este proveedor.</p>
        </div>

        <div class="rounded-2xl border border-white/10 bg-zinc-900/80 p-4">
            <p class="text-[11px] uppercase tracking-[0.2em] text-zinc-400">Pagos registrados</p>
    ...
    <div class="rounded-3xl border border-white/10 bg-zinc-900/80 shadow-[0_24px_60px_rgba(0,0,0,0.45)] p-6 space-y-4">
        <div>
            <p class="text-xs uppercase tracking-[0.3em] text-zinc-500">Pagos y facturas</p>
            ...
            <article class="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-3">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="text-right">
                        <p class="text-lg font-bold text-emerald-200">USD {{ number_format($pago->monto ?? 0, 2) }}</p>
                ...
                @if (!empty($pago->adjuntos))
                    <div class="flex flex-wrap gap-2">
                        @foreach ($pago->adjuntos as $idx => $archivo)
                            <a href="{{ asset('storage/'.$archivo) }}" target="_blank" class="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-[12px] font-semibold text-white hover:border-indigo-400/60">
